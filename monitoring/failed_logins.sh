...
require_commands awk sort; header 'Failed login monitor'; if command -v lastb >/dev/null 2>&1; then lastb -Fi 2>/dev/null | head -200; else journalctl --no-pager --since '7 days ago' 2>/dev/null | grep -Ei 'failed password|authentication failure' | awk '{for (i=1;i<=NF;i++) if ($i == "from") print $(i+1)}' | sort | uniq -c | sort -nr | head -20; fi
